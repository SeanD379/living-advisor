...
function normalizeState(savedState) {
  ...
  return next.household ? { ...next, rulePosts: next.rulePosts || createInitialState().rulePosts, notifications: next.notifications || createInitialState().notifications } : createInitialState();
}
...
function renderLife() {
  ...
  const posts = state.rulePosts.map(post => `<article class="rule-post"><div class="rule-post-head"><div><h3>${post.title}</h3><p>${post.author} 提出</p></div><span class="rule-post-status ${post.adopted ? 'adopted' : ''}">${post.adopted ? '已采纳' : '讨论中'}</span></div><p>${post.text}</p><div class="rule-post-actions"><span>${post.supporters.length} 人支持</span><button class="soft-btn" data-action="support-rule-post" data-id="${post.id}">${post.supporters.includes(me) ? '已支持' : '支持'}</button>${post.author === me && !post.adopted ? `<button class="link-btn" data-action="adopt-rule-post" data-id="${post.id}">采纳</button>` : ''}</div></article>`).join('');
  const content = lifeTab === 'rules'
    ? `<section class="rule-board"><div class="section-head"><div><p class="eyebrow">一起提出、一起决定</p><h2>合租公约</h2></div><button class="link-btn" data-action="create-rule-post">提出公约</button></div>${posts}</section>`
    : lifeTab === 'supplies'
...
function updateHeaderBackLink() {
  ...
  if ((page === 'members' || page === 'notifications') && !existing) topbar.insertAdjacentHTML('afterbegin', '<button class="back-link" data-action="back-to-profile" aria-label="返回我的">‹ <span>我的</span></button>');
