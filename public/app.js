const initial = {
  ...
  rulePosts: [{ id: 1, title: '访客来访提前告知', text: '如有留宿访客，请至少提前一天在这里说明时间和人数。', author: '小王', supporters: ['小王', '小陈'], adopted: true }, { id: 2, title: '厨房使用后的清洁', text: '做完饭后请在当天清理台面和水槽，避免影响下一位室友。', author: '小李', supporters: ['小李'], adopted: false }],
  notifications: { expenses: true, chores: true, supplies: true },
  deletionRequests: [],
  household: {
    adminId: '小陈',
    members: [
...
function normalizeState(savedState) {
  ...
  return next.household ? { ...next, rulePosts: next.rulePosts || fallback.rulePosts, notifications: next.notifications || fallback.notifications, deletionRequests: next.deletionRequests || [] } : fallback;
}
...
function renderLife() {
  ...
  const pendingDelete = (kind, targetId) => state.deletionRequests.find(request => request.kind === kind && request.targetId == targetId);
  const deletionQueue = (kind) => state.deletionRequests.filter(request => request.kind === kind).map(request => `<article class="deletion-request"><div><b>${request.title}</b><p>${request.requestedBy} 申请删除</p></div>${canManage ? `<div><button class="soft-btn" data-action="approve-deletion" data-id="${request.id}">同意删除</button><button class="link-btn" data-action="reject-deletion" data-id="${request.id}">保留</button></div>` : '<span>待管理员审批</span>'}</article>`).join('');
  const supplies = state.supplies.map(x => `<article class="supply"><span class="supply-pic">${x.name === '纸巾' ? '▤' : '◒'}</span><div><h3>${x.name}</h3><p>${x.place} · 剩余 ${x.quantity} ${x.unit}</p></div><div class="supply-actions"><span class="stock ${x.quantity<=x.threshold?'low':''}">${x.quantity<=x.threshold?'待补货':'充足'}</span>${pendingDelete('supply', x.id) ? '<small>待审批</small>' : `<button class="link-btn" data-action="request-delete-supply" data-id="${x.id}">申请删除</button>`}</div></article>`).join('');
  const tabs = `<div class="tabs"><button class="tab ${lifeTab === 'chores' ? 'active' : ''}" data-action="show-life-tab" data-id="chores">值日</button><button class="tab ${lifeTab === 'supplies' ? 'active' : ''}" data-action="show-life-tab" data-id="supplies">公共物品</button><button class="tab ${lifeTab === 'rules' ? 'active' : ''}" data-action="show-life-tab" data-id="rules">公约</button></div>`;
  const posts = state.rulePosts.map(post => `<article class="rule-post"><div class="rule-post-head"><div><h3>${post.title}</h3><p>${post.author} 提出</p></div><span class="rule-post-status ${post.adopted ? 'adopted' : ''}">${post.adopted ? '已采纳' : '讨论中'}</span></div><p>${post.text}</p><div class="rule-post-actions"><span>${post.supporters.length} 人支持</span><button class="soft-btn" data-action="support-rule-post" data-id="${post.id}">${post.supporters.includes(me) ? '已支持' : '支持'}</button>${post.author === me && !post.adopted ? `<button class="link-btn" data-action="adopt-rule-post" data-id="${post.id}">采纳</button>` : ''}${pendingDelete('rule', post.id) ? '<small>待管理员审批</small>' : `<button class="link-btn" data-action="request-delete-rule-post" data-id="${post.id}">申请删除</button>`}</div></article>`).join('');
  const content = lifeTab === 'rules'
    ? `<section class="rule-board"><div class="section-head"><div><p class="eyebrow">一起提出、一起决定</p><h2>合租公约</h2></div><button class="link-btn" data-action="create-rule-post">提出公约</button></div>${posts}${deletionQueue('rule')}</section>`
    : lifeTab === 'supplies'
    ? `<section class="supply-list"><div class="section-head"><h2>公共物品</h2><button class="link-btn" data-action="add-supply">添加物品</button></div>${supplies}${deletionQueue('supply')}</section>`
...
function renderNotifications() { const items = [['expenses', '费用提醒', '新账单、付款进度'], ['chores', '值日提醒', '轮到值日与换班请求'], ['supplies', '物品提醒', '库存不足与补货认领']]; return `<section class="page-title"><div><p class="eyebrow">只接收你关心的消息</p><h2>通知设置</h2></div></section><section class="notification-list">${items.map(([id, title, description]) => `<button class="notification-item" data-action="toggle-notification" data-id="${id}" aria-pressed="${state.notifications[id]}"><span><b>${title}</b><small>${description}</small></span><i class="notification-switch ${state.notifications[id] ? 'on' : ''}" aria-hidden="true"></i></button>`).join('')}</section>`; }
function renderProfile() { return `<section class="profile-card"><span class="profile-avatar">陈</span><div><h3>${houseName()}</h3><p>成员 · 结算日每月 28 日</p></div></section><div class="settings"><button data-action="show-members">成员与邀请 <i>›</i></button><button>默认费用规则 <i>›</i></button><button data-action="show-notifications">通知设置 <i>›</i></button></div><button class="reset" data-action="reset">重置演示数据</button>`; }
function updateHeaderBackLink() {
  const topbar = document.querySelector('.topbar');
  const existing = topbar.querySelector('.back-link');
  if ((page === 'members' || page === 'notifications') && !existing) topbar.insertAdjacentHTML('afterbegin', '<button class="back-link" data-action="back-to-profile" aria-label="返回我的"><span aria-hidden="true">‹</span></button>');
  if (page !== 'members' && page !== 'notifications') existing?.remove();
}
function render() { document.querySelector('.phone-shell').classList.toggle('home-only', page === 'home'); app.innerHTML = page === 'home' ? renderHome() : page === 'expenses' ? renderExpenses() : page === 'life' ? renderLife() : page === 'members' ? renderMembers() : page === 'notifications' ? renderNotifications() : renderProfile(); document.querySelector('.topbar-title h1').textContent = houseName(); updateHeaderBackLink(); const statusLink = document.querySelector('.home-status-link'); if (statusLink) { const model = getHomeModelState(state, me); const pendingCount = [model.debt > 0, model.choreId !== null, model.supplyId !== null, model.ruleId !== null].filter(Boolean).length; statusLink.textContent = pendingCount ? `我的状态 · ${pendingCount}` : '我的状态 · 已完成'; statusLink.setAttribute('aria-label', pendingCount ? `查看我的${pendingCount}项待处理事项` : '查看我的待处理事项，当前已完成'); } document.querySelectorAll('[data-page]').forEach(b => b.onclick = () => go(b.dataset.page)); actionButtons(); }
const actions = {
  'show-life-tab'(tab) { lifeTab = tab; render(); },
  'back-to-profile'() { go('profile'); },
  'show-notifications'() { page = 'notifications'; render(); },
  'toggle-notification'(id) { state.notifications[id] = !state.notifications[id]; save(); render(); },
  'add-supply'() { modal('添加公共物品', `<form id="supply-form"><label for="supply-name">物品名称</label><input id="supply-name" name="name" required maxlength="20" placeholder="例如：洗衣液"><label for="supply-place">存放位置</label><input id="supply-place" name="place" required maxlength="20" placeholder="例如：阳台收纳柜"><label for="supply-quantity">当前数量</label><input id="supply-quantity" name="quantity" required type="number" min="0" value="1"><label for="supply-unit">单位</label><input id="supply-unit" name="unit" required maxlength="6" value="件"><label for="supply-threshold">补货提醒数量</label><input id="supply-threshold" name="threshold" required type="number" min="0" value="1"><button class="primary">添加物品</button></form>`); document.querySelector('#supply-form').onsubmit = event => { event.preventDefault(); const form = new FormData(event.target); state.supplies.push({ id: Date.now(), name: form.get('name').trim(), place: form.get('place').trim(), quantity: Number(form.get('quantity')), unit: form.get('unit').trim(), threshold: Number(form.get('threshold')), claimed: false, history: [] }); save(); closeModal(); lifeTab = 'supplies'; render(); toast('公共物品已添加'); }; },
  ...
  'create-rule-post'() { modal('提出合租公约', `<form id="rule-post-form"><label for="rule-post-title">公约标题</label><input id="rule-post-title" name="title" required maxlength="24" placeholder="例如：公共区域安静时间"><label for="rule-post-text">说明</label><textarea id="rule-post-text" name="text" required rows="4" maxlength="160" placeholder="说说这条公约希望解决什么问题"></textarea><button class="primary">发布提案</button></form>`); document.querySelector('#rule-post-form').onsubmit = event => { event.preventDefault(); const form = new FormData(event.target); state.rulePosts.unshift({ id: Date.now(), title: form.get('title').trim(), text: form.get('text').trim(), author: me, supporters: [me], adopted: false }); save(); closeModal(); lifeTab = 'rules'; render(); toast('公约提案已发布，等室友来支持'); }; },
  'support-rule-post'(id) { const post = state.rulePosts.find(item => item.id == id); if (!post || post.supporters.includes(me)) return; post.supporters.push(me); save(); render(); toast('已支持这条公约提案'); },
  'adopt-rule-post'(id) { const post = state.rulePosts.find(item => item.id == id); if (!post || post.author !== me) return; post.adopted = true; save(); render(); toast('这条公约已采纳'); },
  'request-delete-supply'(id) { const supply = state.supplies.find(item => item.id == id); if (!supply || state.deletionRequests.some(request => request.kind === 'supply' && request.targetId == id)) return; state.deletionRequests.unshift({ id: Date.now(), kind: 'supply', targetId: supply.id, title: supply.name, requestedBy: me }); save(); render(); toast('已提交删除申请，等待管理员审批'); },
  'request-delete-rule-post'(id) { const post = state.rulePosts.find(item => item.id == id); if (!post || state.deletionRequests.some(request => request.kind === 'rule' && request.targetId == id)) return; state.deletionRequests.unshift({ id: Date.now(), kind: 'rule', targetId: post.id, title: post.title, requestedBy: me }); save(); render(); toast('已提交删除申请，等待管理员审批'); },
  'approve-deletion'(id) { if (state.household.adminId !== me) return; const request = state.deletionRequests.find(item => item.id == id); if (!request) return; if (request.kind === 'supply') state.supplies = state.supplies.filter(item => item.id != request.targetId); if (request.kind === 'rule') state.rulePosts = state.rulePosts.filter(item => item.id != request.targetId); state.deletionRequests = state.deletionRequests.filter(item => item.id != id); save(); render(); toast('已同意删除'); },
  'reject-deletion'(id) { if (state.household.adminId !== me) return; state.deletionRequests = state.deletionRequests.filter(item => item.id != id); save(); render(); toast('已保留该内容'); },
