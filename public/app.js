...
function renderHome() {
  ...
  const residentButtons = residents.map(resident => { const residentModel = getHomeModelState(state, resident.id); const residentPending = [residentModel.debt > 0, residentModel.choreId !== null, residentModel.supplyId !== null, residentModel.ruleId !== null].filter(Boolean).length; return `<button class="resident-avatar ${resident.position}" data-action="show-resident" data-resident-id="${resident.id}" data-resident-name="${resident.name}" data-status="${residentPending ? 'pending' : 'settled'}" aria-label="查看${resident.name}的合租信息"><span aria-hidden="true">${resident.name.slice(-1)}</span></button>`; }).join('');
...
function renderProfile() { return `<section class="profile-card"><span class="profile-avatar">陈</span><div><h3>${houseName()}</h3><p>成员 · 结算日每月 28 日</p></div></section><div class="settings"><button>成员与邀请 <i>›</i></button><button>默认费用规则 <i>›</i></button><button>通知设置 <i>›</i></button></div><button class="reset" data-action="reset">重置演示数据</button>`; }
function render() { document.querySelector('.phone-shell').classList.toggle('home-only', page === 'home'); app.innerHTML = page === 'home' ? renderHome() : page === 'expenses' ? renderExpenses() : page === 'life' ? renderLife() : renderProfile(); document.querySelector('.topbar-title h1').textContent = houseName(); const statusLink = document.querySelector('.home-status-link'); if (statusLink) { const model = getHomeModelState(state, me); const pendingCount = [model.debt > 0, model.choreId !== null, model.supplyId !== null, model.ruleId !== null].filter(Boolean).length; statusLink.textContent = pendingCount ? `我的状态 · ${pendingCount}` : '我的状态 · 已完成'; statusLink.setAttribute('aria-label', pendingCount ? `查看我的${pendingCount}项待处理事项` : '查看我的待处理事项，当前已完成'); } document.querySelectorAll('[data-page]').forEach(b => b.onclick = () => go(b.dataset.page)); actionButtons(); }
const actions = {
  ...
  rule(id) { const x = state.rules.find(x => x.id == id); if (x.confirmed.includes(me)) return modal('确认公约', `<p class="rule-text">${x.text}</p><p class="progress">你已确认 · 共 ${x.confirmed.length}/3 位成员确认</p>`); modal('确认公约', `<p class="rule-text">${x.text}</p><p class="progress">已有 ${x.confirmed.length}/3 位成员确认</p><button class="primary" id="confirm">我已阅读并确认</button>`); document.querySelector('#confirm').onclick = () => { if (!x.confirmed.includes(me)) x.confirmed.push(me); save(); closeModal(); render(); toast('公约已确认，感谢共同维护舒适的家'); }; },
